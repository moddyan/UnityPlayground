...
namespace TinyRenderer
{
    ...
    public struct TRColor24
    {
        public byte r;
        public byte g;
        public byte b;

        public static implicit operator Color(TRColor24 c)
        {
            float s = 1 / 255f;
    ...
    public struct FragmentShaderInputData
    {
        ...
        public Color Color;
        public Vector2 UV;
        public NativeArray<TRColor24> TextureData;  //因为我们的纹理都是RGB格式的(24位)，所以不能用Color32(32位)
    ...
    public class ShaderContext
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static Color GetTextureColor(NativeArray<TRColor24> textureData, int w, int h, int x, int y)
        {
            int tidx = y * w + x;
            TRColor24 c = textureData[tidx];
            return (Color)c;
        }
        ...
        public static Color FSBlinnPhong(FragmentShaderInputData input, ShaderUniforms uniforms)
        {
            ...
            if (input.UseBilinear)
            {
                ...
                int u_img_i = (int)(u_img);  // 整数部分
                int u0 = u_img < u_img_i + 0.5 ? u_img_i - 1 : u_img_i;  // u0, u1分别是u_img的前后整数
                if (u0 < 0) u0 = 0;
                int u1 = u0 + 1;
            ...
            Color ks = new Color(0.7937f, 0.7937f, 0.7937f);  // TODO, Magic Number
